...
function pushMetric(action, log, metricObj) {
    ...
    console.log(JSON.stringify(metricObj, null, 4));
    ...
    const utapiObj = {
        ...
        byteLength,
        numberOfObjects,
        ts: new Number(new Date()) * 1000,
        req_id: log.getSerializedUids(),
    };

    if (newByteLength !== undefined && oldByteLength !== undefined) {
        utapiObj.byteDelta = newByteLength - oldByteLength;
    }

    if (byteLength === undefined && newByteLength) {
        utapiObj.byteLength = newByteLength;
    } else if (byteLength === undefined) {
        utapiObj.byteLength = 0;
    }
    ...
    request.post('http://localhost:8082/stats', {body:utapiObj, json:true}, function (err, res, body) {
        if (err) {
            return;
        }
        log.debug(`pushed metric: ${res.statusCode}`);
