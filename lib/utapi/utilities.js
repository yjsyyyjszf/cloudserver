...
const { Client: UtapiClient } = require('utapiv2');
...
function pushMetric(action, log, data) {
    const { bucket, keys, byteLength, newByteLength,
                    oldByteLength, numberOfObjects, authInfo, canonicalID } = data;
    const metric = {
        operationId: action,
        timestamp: + new Date(),
        bucket,
        object: numberOfObjects === 1 ? keys[0] : undefined,
        count: numberOfObjects,
        bytes: newByteLength,
        account: canonicalID
    }
    
    if (Number.isInteger(newByteLength) && Number.isInteger(oldByteLength)) {
        metric.bytesDelta = newByteLength - oldByteLength;
    }

    if (authInfo) {
        metric.account = authInfo.getCanonicalID()
        metric.user = authInfo.isRequesterAnIAMUser() ? authInfo.getShortid() : undefined;
    }
    utapi.pushMetric(metric);
}
