...
function _storeIt(bucket, objectKey, objMD, authInfo, canonicalID, cipherBundle,
                  request, streamingV4Params, log, corsHeaders, callback) {
                ...
                return _storeInMDandDeleteData(
                    ...
                    logger.newRequestLoggerFromSerializedUids(
                        log.getSerializedUids()), (err, contentMD5) => {
                            ...
                            return callback(null, contentMD5, corsHeaders);
                        });
            });
    }
...
function objectPut(authInfo, request, streamingV4Params, log, callback) {
    ...
    return services.metadataValidateAuthorization(valParams, (err, bucket,
        ...
        if (bucket.hasDeletedFlag() &&
            ...
            return callback(errors.NoSuchBucket, null, corsHeaders);
        }
            ...
            return cleanUpBucket(bucket,
                    canonicalID, log, err => {
                        ...
                        return _storeIt(bucket, objectKey, objMD,
                                        ...
                                        streamingV4Params, log, corsHeaders,
                                (err, contentMD5) =>
            ...
            return kms.createCipherBundle(
                ...
                log, (err, cipherBundle) => {
                    ...
                    return _storeIt(bucket, objectKey, objMD,
                                    ...
                                    corsHeaders, (err, contentMD5) =>
                                        callback(err, contentMD5, corsHeaders));
                });
        }
        return _storeIt(bucket, objectKey, objMD, authInfo, canonicalID, null,
                        request, streamingV4Params, log, corsHeaders, callback);
    });
}
