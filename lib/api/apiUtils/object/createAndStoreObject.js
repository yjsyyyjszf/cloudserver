...
const { indexVideo } = require('./azureIndexer.js');
...
function createAndStoreObject(bucketName, bucketMD, objectKey, objMD, authInfo,
    ...
    return async.waterfall([
        function indexAzureVideo(next) {
            const fileExtension = objectKeyContext.objectKey.split('.').pop();
            if (config.azureExtensions.indexOf(fileExtension) === -1) {
                return next(null, request);
            }
            return indexVideo(request, objectKeyContext, log,
            (err, stream2) => {
                if (err) {
                    return next(err);
                }
                return next(null, stream2);
            });
        },
        function storeData(stream2, next) {
            if (size === 0 && !dontSkipBackend[locationType]) {
                metadataStoreParams.contentMD5 = constants.emptyFileMd5;
                return next(null, null, null);
            }
            return dataStore(objectKeyContext, cipherBundle, stream2, size,
