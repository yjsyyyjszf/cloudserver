...
class Config extends EventEmitter {
    ...
    _getConfig() {
        ...
        if (config.azureIndexer) {
            if (config.azureIndexer.extensions &&
                config.azureIndexer.extensions.length > 0) {
                // TODO: add assertions
                this.azureExtensions = config.azureIndexer.extensions;
            } else {
                this.azureExtensions = [];
            }
            this.azureLocation = process.env.AZURE_LOCATION ||
              config.azureIndexer.location;
            this.azureAccountId = process.env.AZURE_ACCOUNT_ID ||
              config.azureIndexer.accountId;
            this.azurePrimaryKey = process.env.AZURE_PRIMARY_KEY ||
              config.azureIndexer.primaryKey;
        }
    }
