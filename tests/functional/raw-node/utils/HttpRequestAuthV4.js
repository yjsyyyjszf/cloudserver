...
${signedHeadersList.map(header => `${header}:${signedHeaders[header]}\n`).join('')}
...
${signedHeaders['x-amz-content-sha256']}`;
    ...
    getAuthorizationHeader(urlObj, signedHeaders) {
        ...
        return `AWS4-HMAC-SHA256 Credential=${this._accessKey}/${this.getCredentialScope()}, SignedHeaders=${signedHeadersList.join(';')}, Signature=${authorizationSignature}`;
    }
    ...
    _constructRequest(hasDataToSend) {
        ...
        this._timestamp = `${isoDate.slice(0, 4)}${isoDate.slice(5,7)}${isoDate.slice(8,13)}${isoDate.slice(14,16)}${isoDate.slice(17,19)}Z`;
    ...
    _write(chunk, encoding, callback) {
        ...
        this._httpRequest.once('drain', callback);
    }
}
