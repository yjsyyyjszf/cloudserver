...
const azureBlobEndpoint =
  process.env[`${azureLocation}_AZURE_BLOB_ENDPOINT`] ?
  process.env[`${azureLocation}_AZURE_BLOB_ENDPOINT`] :
  config.locationConstraints[azureLocation].details.azureBlobEndpoint;
const azureBlobSAS =
  process.env[`${azureLocation}_AZURE_BLOB_SAS`] ?
  process.env[`${azureLocation}_AZURE_BLOB_SAS`] :
...
const azureClient = azure.createBlobServiceWithSas(azureBlobEndpoint,
  azureBlobSAS);
            ...
            'key, and newest object should be returned', done => {
                ...
                async.waterfall([
                    ...
                    next => {
                        ...
                        s3.putObject(params, err => next(err));
                    },
                    next => {
                        setTimeout(() => {
